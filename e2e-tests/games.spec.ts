...
test.describe("Game Listing and Navigation", () => {
  ...
  test("should filter by category alone", async ({ page }) => {
    await page.goto("/");
    await page.getByTestId("category-filter-1").check();
    await page.getByTestId("apply-filters").click();
    ...
    await expect(page.getByTestId("filter-summary")).toHaveText(
      "Showing 4 games matching your filters.",
    );
  });

  test("should filter by publisher alone", async ({ page }) => {
    await page.goto("/");
    await page.getByTestId("publisher-filter").selectOption({ label: "CodeForge Studios" });
    await page.getByTestId("apply-filters").click();

    await expect(page).toHaveURL(/publisher=CodeForge(?:%20|\+)Studios/);
    await expect(page.locator('[data-testid="game-card"]:not([hidden])')).toHaveCount(6);
    await expect(page.getByTestId("filter-summary")).toHaveText(
      "Showing 6 games matching your filters.",
    );
  });

  test("should show an empty state for unmatched filters", async ({ page }) => {
    await page.goto("/?category=Strategy&publisher=Unknown");
    ...
    await expect(page.getByTestId("filter-summary")).toHaveText(
      "Showing 0 games matching your filters.",
    );
  });
